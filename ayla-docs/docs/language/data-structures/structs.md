...
there are two types of `structs` in Ayla , typed structs, and anonymous structs
...
you can just initialise them with `struct {}`
...
### Type Safety in Anonymous Structs

field types are inferred from their initial values.
...
oper.Left = "hi" // string
```
> output: runtime error at 6:17: field 'Left' expected 'int' but got 'string'
